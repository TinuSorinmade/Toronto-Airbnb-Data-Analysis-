...
```{r data}
library(readxl)
airbnb <- read_excel("Airbnb-Canada.xlsx")
```
...
![](http://127.0.0.1:18919/chunk_output/s/85393379/c2ci8xvoc1637/000012.png)
...
data_animation <- data.frame(
  Time = rep(1:5, each = 2),  # Time variable for animation
  Category = rep(c("Below Median Revenue", "Above Median Revenue"), times = 5),  # Categories
  Count = c(50, 30, 60, 40, 70, 50, 80, 60, 90, 70)  # Count for each category and time
)

# Create a base ggplot object for the animation
p <- ggplot(data_animation, aes(x = Category, y = Count, fill = Category)) +
  geom_bar(stat = 'identity', position = 'dodge') +  # Create bars for each category
  labs(title = 'Distribution of Properties over Time', x = 'Revenue Classification', y = 'Count') +  # Add labels
  theme_minimal()  # Use a minimal theme

# Add animation layers
p <- p +
  transition_time(Time) +  # Set the time variable for animation
  ease_aes('linear')  # Use linear easing for smooth transitions

...
airbnb_new <- cbind(airbnb, yearly_revenue)  # Append the yearly revenue column to a new version of the airbnb data set
```
...
test <- airbnb_new2[-trainingSamples, ]  # Extract the testing data
```
...
pR2(model)  # Ensure that the pR2 function is properly loaded (usually from the `pscl` package)
```
